...
// User functions
async function getUsers() { try { return JSON.parse(await fs.readFile(usersFile, 'utf8')); } catch { return {}; } }
async function saveUsers(users) { await fs.writeFile(usersFile, JSON.stringify(users, null, 2)); }
async function getGames() { try { return JSON.parse(await fs.readFile(gamesFile, 'utf8')); } catch { return {}; } }
async function saveGames(games) { await fs.writeFile(gamesFile, JSON.stringify(games, null, 2)); }
...
async function createUser(userId, userData) {
    ...
    if (!users[userId]) {
        users[userId] = {
            ...
            balance: parseInt(process.env.INITIAL_BALANCE) || 10,
            created_at: new Date().toISOString(),
            last_login: new Date().toISOString(),
            games_played: 0,
            games_won: 0,
            total_earned: parseInt(process.env.INITIAL_BALANCE) || 10,
            is_active: true
        };
        ...
        if (result) {
            ...
            if (chess.isGameOver()) {
                ...
                if (chess.isCheckmate()) {
                    ...
                    const reward = parseInt(process.env.WIN_REWARD) || 10;
...
💰 *Get ${process.env.INITIAL_BALANCE || 10} birr FREE* for registering!
🏆 *Win ${process.env.WIN_REWARD || 10} birr* for every game you win!
...
3. Get your ${process.env.INITIAL_BALANCE || 10} birr bonus
4. Start playing and earning!

Click below to begin! 🎮`;

    // IMPORTANT: Use your Render URL here
    const webAppUrl = `https://free-n8wq.onrender.com`;
    
    const keyboard = {
        inline_keyboard: [[
            {
                text: "🎮 Register & Play",
                web_app: { url: `${webAppUrl}?start=${userId}` }
            }
        ]]
    };
...
async function start() {
    ...
    app.listen(PORT, () => {
        ...
        console.log(`💰 Bonus: ${process.env.INITIAL_BALANCE || 10} birr`);
        console.log(`🏆 Win: ${process.env.WIN_REWARD || 10} birr`);
    });
}
